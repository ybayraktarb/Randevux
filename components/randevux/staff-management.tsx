...
import { Loader2, Plus, MoreHorizontal, Search, Calendar, ShieldAlert, Users, Settings2 } from "lucide-react"
...
import { StaffDetail } from "@/components/randevux/staff-detail"
...
export function StaffManagement() {
    ...
    const [addForm, setAddForm] = useState({ name: "", email: "", phone: "", password: "", role: "staff" })
    ...
    async function handleAddStaff() {
        if (!addForm.name || !addForm.email || !addForm.password) {
            toast.error("İsim, E-posta ve Şifre alanları zorunludur.")
            return
        }
        ...
        try {
            // KALDIRILDI: const { createStaffAction } = await import('@/app/actions/staff.actions')
            const result = await createStaffAction(formData)

            if (!result.success) {
                toast.error(result.error.message) // DEĞİŞTİRİLDİ — ActionResult formatı
            ...
            setAddForm({ name: "", email: "", phone: "", password: "", role: "staff" })
        ...
        <div className="flex flex-col gap-6">
            ...
            {loading ? (
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {filteredStaff.map((s) => (
                        <div key={s.id} className="group relative flex flex-col rounded-xl border border-border bg-card p-5 shadow-sm transition-all hover:shadow-md hover:border-primary/40">
                            <div className="flex flex-col items-center text-center mt-2">
                                <RxAvatar name={s.user?.name} src={s.user?.avatar_url} size="lg" className="mb-3" />
                                <h3 className="text-base font-semibold text-foreground line-clamp-1">{s.user?.name || "Profil Bekleniyor..."}</h3>
                                <p className="text-xs text-muted-foreground mt-0.5 line-clamp-1">{s.user?.email || "Senkronize ediliyor..."}</p>
                            ...
                            <div className="mt-4 border-t border-border pt-4 flex items-center justify-between text-xs text-muted-foreground">
                                ...
                                >
                                    <Settings2 className="size-3.5" /> Yönet
                                </RxButton>
                            </div>
                        </div>
                    ))}
