    ...
    useEffect(() => {
        if (acik) {
            ...
            if (duzenlenecekPaket) {
                ...
                setSektorId(duzenlenecekPaket.sector_id)
...
export function PackagesTab() {
    ...
    const filtrePaketler = paketler.filter((p) => {
        ...
        if (aktifSektorTab === "genel") return !p.sector_id
        return p.sector_id === aktifSektorTab
    })

    // Sektör bazlı istatistikler
    const sektorStats = (sId: string | null) => {
        const list = sId ? paketler.filter((p) => p.sector_id === sId) : paketler.filter((p) => !p.sector_id)
