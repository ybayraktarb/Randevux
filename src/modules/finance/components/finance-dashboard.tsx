...
} from "@/src/modules/finance/actions/finance.actions"
...
function KasaHareketleriTab({ businessId }: { businessId: string }) {
    ...
    const fetchData = useCallback(async () => {
        ...
        if (res.success && (res as any).data) {
            ...
            toast.error((res as any).error || "Giderler yüklenemedi")
        }
        setLoading(false)
    }, [businessId])
                                                ...
                                                onClick={async () => {
                                                    if (confirm("Bu işlemi silmek istediğinize emin misiniz?")) {
                                                        ...
                                                        if (res.success) {
                                                            ...
                                                            toast.error((res as any).error || "Silinemedi")
                                                        }
                                                    }
                                                }}
...
function AddTransactionModal({ open, onClose, businessId, onAdded }: { open: boolean, onClose: () => void, businessId: string, onAdded: () => void }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSaving(false)
        if (!res.success) {
            toast.error((res as any).error || "Eklenemedi")
            return
        }
...
function UpdateCommissionModal({ open, onClose, staffData, onUpdated }: { open: boolean, onClose: () => void, staffData: any, onUpdated: () => void }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (res.success) {
            ...
            toast.error((res as any).error || "Güncellenemedi")
        }
    }
...
function HakEdisRaporlariTab({ businessId }: { businessId: string }) {
    ...
    useEffect(() => {
        ...
        getStaffCommissionsAction(businessId).then(res => {
            if (res.success && (res as any).data) setStaffList((res as any).data)
        })
    }, [businessId])
    ...
    const handleCalculate = async () => {
        ...
        if (res.success && (res as any).data) {
            setPreviewData((res as any).data)
            toast.success("Önizleme oluşturuldu")
        } else {
            toast.error((res as any).error || "Hesaplanamadı")
        }
    ...
    const handleSavePayroll = async () => {
        ...
        if (res.success) {
            ...
            toast.error((res as any).error || "Ödeme yapılamadı")
        }
    }
