...
export class AppointmentService {
    ...
    static async createAppointment(input: CreateManualAppointmentInput): Promise<{ success: boolean; appointmentId?: string; error?: { message: string } }> {
        try {
            ...
            if (!finalCustomerId && data.guestName && data.guestPhone) {
                ...
                if (!existingCust) {
                    await supabaseAdmin.from("business_customers").insert({
    ...
    static async addAppointmentNote(input: { appointmentId: string; businessId: string; note: string; userId: string }): Promise<{ success: boolean; error?: { message: string } }> {
        try {
            const supabaseAdmin = await createAdminClient()
            const { data: staffRow } = await supabaseAdmin
                .from("staff_business")
            ...
            if (!staffRow) throw new Error("Personel yetkisi bulunamadı.")
            ...
            if (!aptData) throw new Error("Randevu bulunamadı.")

            const { error: insertError } = await supabaseAdmin.from("customer_notes").insert({
                business_id: input.businessId,
                customer_user_id: aptData.customer_user_id,
    ...
    static async bulkUpdateStatus(input: { appointmentIds: string[]; status: string; businessId: string }, initiatorRole: string): Promise<{ success: boolean; error?: { message: string } }> {
        try {
            const statusMap: Record<string, string> = {
                "pending": "Bekliyor",
                "confirmed": "Onaylandı",
            ...
            if (mappedStatus === "Onaylandı") updatePayload.confirmed_at = now
            if (mappedStatus === "Tamamlandı") updatePayload.completed_at = now
            if (mappedStatus === "İptal") {
                updatePayload.cancelled_by = initiatorRole
                updatePayload.cancelled_at = now
            }
            ...
            Sentry.captureException(err)
            return { success: false, error: { message: "Toplu güncelleme başarısız." } }
        }
    }
}
