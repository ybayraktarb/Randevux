...
export class StaffService {
  ...
  static async create(input: CreateStaffInput): Promise<{ success: boolean; data?: { user: any }; error?: { message: string } }> {
    ...
    const { data: limitData } = await supabaseAdmin
      .from('subscriptions')
      .select(`packages ( max_staff )`)
      .eq('business_id', input.businessId)
      .eq('is_active', true)
      .eq('status', 'active')
      .single();
      
    const maxStaff = (limitData?.packages as any)?.max_staff;
    ...
    if (maxStaff !== undefined && maxStaff !== null) {
      ...
      if (count !== null && count >= maxStaff) {
         return { success: false, error: { message: `Personel eklenemedi: Paketiniz en fazla ${maxStaff} personele izin vermektedir.` } };
      }
    }
    // ─── Limit Kontrolü Sonu ───
